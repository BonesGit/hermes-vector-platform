...
1. Resolve `vector-bridge` (`VECTOR_BRIDGE_BIN`, an in-tree `bridge/target/release/vector-bridge`, or an installed prebuilt under `plugin-data/vector-platform/bin/`). If missing **or the `.version` stamp does not match this plugin**, download `vector-bridge-<triple>` + `SHA256SUMS` from the `v{plugin version}` GitHub Release (Linux / macOS, x86_64 and aarch64). Cargo (`cd bridge && cargo build --release --locked`, Rust ≥ 1.75) is the fallback. Download and build happen **only** in setup, never at `hermes gateway start`. Gateway start still uses a previously installed prebuilt if the stamp lags (so a Python-only plugin update keeps working); re-run setup to refresh it.
...
| `vector-bridge` binary not found | Run `hermes gateway setup`. It downloads a Release asset into `plugin-data/vector-platform/bin/` (Linux / macOS) or `cargo build --release --locked` in `bridge/`. `VECTOR_BRIDGE_BIN` overrides. `hermes gateway start` does **not** download or compile. After a plugin update, start still uses a previously installed prebuilt if `.version` lags. |
